 ...
 * A stamp added while the card is on screen (at the desk) pops in and is read out.
 */
export function StampCard({ status, size = 'lg', className }: StampCardProps) {
  ...
  // Stamps above `fresh` arrived while the card was showing (null: none yet, or a stamp came off).
  const [shown, setShown] = useState(status.stamps);
  const [fresh, setFresh] = useState<number | null>(null);
  const [changed, setChanged] = useState(false);
  if (status.stamps !== shown) {
    setShown(status.stamps);
    setFresh(status.stamps > shown ? shown : null);
    setChanged(true);
  }
  const popped = (visit: number) => fresh !== null && visit > fresh && visit <= status.stamps;
  ...
  return (
    <>
              ...
              <li
                ...
                className={cx(
                  ...
                  stamped
                    ? 'bg-rose-500'
                    : percent
                      ? 'bg-white ring-2 ring-inset ring-rose-400'
                      : 'bg-white/80 ring-1 ring-inset ring-ink-200',
                  popped(visit) && 'animate-pop',
                )}
              />
            );
          }
      ...
      {/* The new count, read out when staff add or remove a stamp while the card is open. */}
      {lg ? (
        <span className="sr-only" role="status">
          {changed ? t('stamps', { stamps: status.stamps, cycle: status.cycle }) : ''}
        </span>
      ) : null}
    </>
  );
